...
function parse<T = Record<string, any>>(text: string, options?: VDFParseOptions | boolean): T {
    if (typeof text !== "string") {
        throw new TypeError("VDF.parse: Expecting parameter to be a string");
    }

    const opts: VDFParseOptions = {
        types:
            (typeof options === "boolean") ? options // backward compatibility with the old boolean param
                : ((typeof options === "object" && options !== null && "types" in options) ? (options as VDFParseOptions).types : true),
        arrayify: (typeof options === "object" && options !== null && "arrayify" in options) ? (options as VDFParseOptions).arrayify : true,
        conditionals: (typeof options === "object" && options !== null && "conditionals" in options) ? (options as VDFParseOptions).conditionals : undefined
    };

    if (opts.conditionals && !Array.isArray(opts.conditionals) && typeof opts.conditionals === "string") opts.conditionals = [opts.conditionals];
    ...
    const getNextLine = function (): string | false {
        if (sublines && sublines.length) {
            const _subline = sublines.shift();
            if (!odd && _subline) return _subline.trim(); // we need to trim the line if outside of quoted value
            return _subline || false;
        }
        ...
        let comment_slash_pos = -1;
        sanitization: for (let l = 0; l < _line.length; l++) {
    ...
    while ((line = getNextLine()) !== false) {
        ...
        if (line == "" || (typeof line === 'string' && line[0] == '/')) { continue; }
        ...
        while (typeof line === 'string') {
            ...
            let val: string | number | boolean = (m[6] !== undefined) ? m[6] : m[8];
            ...
            if (val === undefined) {
                ...
                if (opts.conditionals !== undefined && Array.isArray(opts.conditionals) && m[9]) {
                    let conditionals = m[9];
                    const single_cond_regex = new RegExp('^(\\|\\||&&)?(!)?\\$([A-Z0-9]+)');
                    let ok = false;
                    while (conditionals) {
                        const d = single_cond_regex.exec(conditionals);
                        if (d === null || !d[3])
                        ...
                        const not = d[2] && d[2] === '!';
                        const cond = d[3];
                        const includes = opts.conditionals.indexOf(cond) !== -1;
                        const _ok = not ? !includes : includes;
                        if (!op || op === '||')
                            ok = ok || _ok;
                        else // &&
                            ok = ok && _ok;
...
function stringify(obj: object, options?: VDFStringifyOptions | boolean): string {
    if (typeof obj !== "object" || obj === null) {
        throw new TypeError("VDF.stringify: First input parameter is not an object");
    }

    const opts: VDFStringifyOptions = {
        pretty:
            (typeof options === "boolean") ? options // backward compatibility with the old boolean param
                : ((typeof options === "object" && options !== null && "pretty" in options) ? (options as VDFStringifyOptions).pretty : false),
        indent: (typeof options === "object" && options !== null && "indent" in options) ? (options as VDFStringifyOptions).indent : "\t"
    };

    return _dump(obj, opts, 0);
}

function _dump(obj: Record<string, any>, options: VDFStringifyOptions, level: number): string {
    if (typeof obj !== "object" || obj === null) {
...
export type { VDFParseOptions, VDFStringifyOptions };
