package cn.javat.csa.cms.common.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseBody<List<String>> handleValidationError(MethodArgumentNotValidException e) {

        // 🔥 拿到所有缺失/为空的字段名数组
        List<String> missingFields = e.getBindingResult().getFieldErrors().stream()
                .map(FieldError::getField)
                .toList();
        return ResponseBody.<List<String>>builder()
                .code(400)
                .message("缺少必要参数")
                .data(missingFields)
                .build();
